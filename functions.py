...
def kernel_derivatives(x1, x2, u_noisy, kernel, order=1, **kwargs):
    x1 = x1[:, None]
    x2 = x2[:, None]
    if kernel == 'rbf':
...
def functional_form_PDE(s, s_test, fx, kernel: str, **kwargs):
    if kernel == 'rbf':
        length_scale = kwargs.get('l')
        K = RBF(length_scale=length_scale)
        P = K(s_test, s) @ solve((K(s, s) + 0.01 * np.eye(s.shape[0])), fx)
        return P
    if kernel == 'poly':
        c = kwargs.get('coef0')
        d = kwargs.get('degree')
...
def kernel_smoothing(u_noisy, x, kernel: str, **kwargs):
    x = x[:, None]
    Nx = len(x)
    if kernel == 'rbf':
        length_scale = kwargs.get('l')
    ...
    u_smooth = U @ solve((U + 0.1 * np.eye(Nx)), u_noisy)
    return u_smooth
