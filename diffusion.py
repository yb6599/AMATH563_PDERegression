...
def smooth_rbf_kernel(u_diff, u_diff_noisy, x, l, Nx):
    # RBF kernel smoothing function
    rbf = kernels.RBF(length_scale=l)
    u_rbf = rbf(x[:, None], x[:, None])
    u_diff_smooth = u_rbf @ solve((u_rbf + 0.1 * np.eye(Nx)), u_diff_noisy)
    
    mse_u = calculate_mse(u_diff, u_diff_smooth)
    return mse_u, u_diff_smooth, u_rbf

def compute_derivatives_and_evaluate(u_diff_smooth, u_rbf, x, l, Nx):
    f = np.cos(2 * np.pi * x)
    ...
    mse = calculate_mse(f, f_new)
    return mse, u_kx, u_kxx


def smooth_poly_kernel(u_diff, u_diff_noisy, x, deg, coef, gam, Nx):
    ...
    mse_u = calculate_mse(u_diff, u_diff_smooth)
    return mse_u, u_diff_smooth

def compute_derivatives_and_evaluate_poly(u_diff_smooth, x, deg, coef, gam, Nx):
    f = np.cos(2 * np.pi * x)
    sk = sklearn.metrics.pairwise.polynomial_kernel(x[:, np.newaxis], x[:, np.newaxis], degree=deg, coef0=1, gamma=1)
